<template>
  <div
    class="ongoing-campaigns"
    v-if="ongoingCampaigns.length > 0 && campaignHeading"
  >
    <div v-if="$i18n.locale == 'en'" class="campaigns-details container">
      <h1 class="campaigns-title">
        {{ campaignHeading.title }}
      </h1>
      <p class="campaigns-description">
        {{ campaignHeading.sub_title }}
      </p>
    </div>
    <div v-if="$i18n.locale == 'bn'" class="campaigns-details container">
      <h1 class="campaigns-title">
        {{ campaignHeading.title_bangla }}
      </h1>
      <p class="campaigns-description">
        {{ campaignHeading.sub_title_bangla }}
      </p>
    </div>

    <div class="campaigns-card">
      <a
        :href="`/campaign-details/?campaign=${singleItem.slug}`"
        v-for="singleItem in ongoingCampaigns"
        :key="singleItem.id"
        class="ongoing"
        v-if="singleItem.images.length > 0"
        :style="{
          backgroundImage: `url(${singleItem.images[0].image_url})`,
        }"
        data-aos="fade-up"
        data-aos-delay="300"
        data-aos-duration="600"
      >
        <div class="need interest-image">
          <img src="~assets/image/logo/footerLogo.png" alt="" />
        </div>
        <div class="rate interest-text">
          <nuxt-link
            :to="{
              path: `${localePath('/campaign-details/')}`,
              query: { campaign: singleItem.slug },
            }"
          >
            <h3 v-if="$i18n.locale == 'en'">{{ singleItem.title }}</h3>
            <h3 v-if="$i18n.locale == 'bn'">{{ singleItem.title_bangla }}</h3>
            <div class="ongoing-campaign-circle">
    ...
    <div class="campaigns-button">
      <nuxt-link to="/campaign">
        <button class="more-campaigns">
          {{ $i18n.locale == "en" ? "more campaigns" : "আরো প্রচারণা" }}
  ...
  methods: {
    async getOngoingCampaigns() {
      const response = await this.$axios.get(
        `/get-ongoing-campaign/${this.page}`
      );
      this.ongoingCampaigns = response.data.data;
      this.campaignHeading = response.data.campaignHeading[0];
    },
  },
};
</script>
...
.ongoing-campaigns {
  position: relative;
  background: rgba(253, 225, 240, 0.2);
