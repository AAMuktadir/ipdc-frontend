          ...
          <div class="form-group-row">
            <div class="form-group">
              ...
              <input
                type="date"
                class="form-control"
                :class="{ error: errors.date && errors.date.length > 0 }"
                id="date"
                placeholder="Booking Date"
                v-model="date"
                :min="today"
              />
              <div v-if="errors.date.length > 0" class="errors">
                <ul>
                  <li v-for="error in errors.date" :key="error">
                    {{ error }}
                  </li>
                </ul>
              </div>
            ...
            <div class="form-group">
              ...
              <select
                id="timeslots"
                class="form-control"
                :class="{ error: errors.timeslots.length > 0 }"
                v-model="timeslots"
              >
                <option key="0" value="">Select</option>
                <option
              ...
              <div v-if="errors.timeslots.length > 0" class="errors">
                <ul>
                  <li v-for="error in errors.timeslots" :key="error">
                    {{ error }}
                  </li>
                </ul>
              </div>
          ...
          <div class="form-group">
            <div class="form-check">
              <input
                ...
                :class="{ error: errors.consent.length > 0 }"
                type="checkbox"
                id="consent"
                v-model="consent"
              />
            ...
            <div v-if="errors.consent.length > 0" class="errors">
              <ul>
                <li v-for="error in errors.consent" :key="error">
                  {{ error }}
                </li>
              </ul>
            </div>
...
export default {
  data() {
    return {
      ...
      timeSlotList: [
        { id: 1, value: "10AM-11AM", title: "10AM-11AM" },
        { id: 2, value: "11AM-12PM", title: "11AM-12PM" },
        { id: 3, value: "12PM-1PM", title: "12PM-1PM" },
        { id: 4, value: "1PM-2PM", title: "1PM-2PM" },
        { id: 5, value: "2PM-3PM", title: "2PM-3PM" },
        { id: 6, value: "3PM-4PM", title: "3PM-4PM" },
        { id: 7, value: "4PM-5PM", title: "4PM-5PM" },
      ],
    };
  },
  computed: {
  ...
  methods: {
    ...
    checkForm() {
      ...
      if (!this.date) {
        this.errors.date.push("Select Date.");
      }
      if (!this.timeslots) {
        this.errors.timeslots.push("Select Time Slot.");
      }
      if (!this.consent) {
        this.errors.consent.push("Consent is required.");
      }
      ...
      return (
        ...
        this.errors.timeslots.length === 0 &&
        this.errors.consent.length === 0
      );
    },
    async submitHandler() {
      try {
        ...
        if (toekn && this.checkForm()) {
          const postData = {
            ...
            schedule_time_slot: this.timeslots,
          };
