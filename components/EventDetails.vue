<template>
  <section class="event-details">
    ...
    >
      ...
      <div class="event-experience">
        <div v-if="$i18n.locale == 'en'" class="event-list">
          <nuxt-link to="/signature-events">Events<span>/</span></nuxt-link>
          <nuxt-link to="/event-details">Event Details</nuxt-link>
        </div>
        <div v-if="$i18n.locale == 'bn'" class="event-list">
          <nuxt-link to="/bn/signature-events">Events<span>/</span></nuxt-link>
          <nuxt-link to="/event-details">Event Details</nuxt-link>
        </div>
        <div class="event-experience-heading">
          <h1>
            {{
              $i18n.locale == "en" ? singleEvent.title : singleEvent.title_bn
            }}
          </h1>
        </div>

        <b-row v-if="singleEventImages.length > 0" align-h="center">
          <b-col
            v-for="singleImage in singleEventImages"
            :key="singleImage"
            lg="6"
            md="8"
          >
            <div
              class="event-experience-image"
              :style="{ backgroundImage: `url(${singleImage.image})` }"
            />
          </b-col>
        </b-row>

        <div class="event-experience-description">
          <div v-if="$i18n.locale == 'en'" v-html="singleEvent.description" />
          <div v-else v-html="singleEvent.description_bn" />
        </div>
      </div>
    </div>
  </section>
